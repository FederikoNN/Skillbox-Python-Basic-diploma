...
async def command_low_price(message: types.Message, state: FSMContext):
    try:
        # Записываем действия пользователя в лог файл, пишем его id и команду, которую он ввел
        logging.info(f'{message.from_user.id} {message.text}')
        # Сбрасываем состояние, это необходимо сделать, чтобы пользователь смог зайти в команду из любого состояния
        # иначе ответа не будет совсем и он останется в том же состоянии их которого пытался перейти
        await state.finish()
        ...
        # При возникноверии исключения, записываем его в лог файл с пометкой исключение
        logging.exception(e)
        await state.finish()
...
async def price_range(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer('Введите диапазон цен в рублях, в формате: 700-2000')
...
# Регулярым выражением ограничиваем входные данные, а именно 'любое число-любое число'
...
async def distance_range(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer('Введите диапазон расстояния, на котором находится отель от центра в км, в формате: 1-5')
...
async def input_city_func(message: types.Message, state: FSMContext):
    try:
        ...
        if number_of_hotels > 0:
            sorted_hotels = []
            for hotel in hotels:
                # Берем данные по расстоянию от центра города до отеля в цифре, входные данные типа: 3,4 км
                hotel_ditance = float(hotel['landmarks'][0]['distance'].split(' ')[0].replace(',', '.'))
                # Если расстояние от отеля до центра города попадает под заданные от пользователя
                if (hotel_ditance >= start_dist) and (hotel_ditance <= finish_dist):
                    sorted_hotels.append(hotel)
                    valid_hotels_counter += 1
            # Если нашли хоть один отель после сортировки по заданным параметрам, то заносит данные в машину состояния
...
async def show_low_price_hotels(call: CallbackQuery, state: FSMContext):
    try:
        ...
        # Из колбека (то что передается через кнопку) берем состояние (photos_yes или photos_no)
        ...
        for hotel in hotels:
            try:
                if counter_hotel != hotels_amount:
                    # Создаем медиагруппу для ТГ поста, в нее могут входить не только фотки
                    album = MediaGroup()
                    hotel_id = hotel['id']
                    hotel_name = hotel['name']
                    hotel_address = hotel['address']['streetAddress']
                    ...
                    for photo in photos_mass:
                        if photo_amount != counter_photo and photos_mass.index(photo) < 10:
                            counter_photo += 1
                            # Изначально ссылка неликвидная, туда вместе {size} навдо подставить букву, указываю размер фотки
                            photo_link = photo['baseUrl']
                            # Берем фотку с параметром z, это определенный размер, там другие буквы - другие разрешения фоток
                            ...
                            if (photo_amount - counter_photo) == 0:
                                hotel_description = "Название отеля: {name}\n" \
                                                    ...
                                                    "Цена за сутки: {price} \n\n".format(name=hotel_name,
                                                                                         ...
                                                                                         distance=hotel_distance,
                                                                                         price=hotel_price)
                                # Добавляем в пост фото, здесь с названием, так как
